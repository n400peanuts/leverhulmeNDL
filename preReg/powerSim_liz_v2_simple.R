...
means_lists = list(list(alpha,beta1,beta2/2,beta3/2))
...
# check for singularity and convergence warnings
      ...
      # run a version of the model where we fit an intercept for each of the fixed effects amd extract those effects
       ...
       model1.emm.con <-contrast(model1.emm, "consec",  simple = "each", combine = T, adjust = "bonferroni")
...
# now for simple effect of learning condition in low
      ...
      for (H1_simple_effect in H1s_simple_effect) {
          ...
          if (is.na(BF)) {BF_cat = "NA"} else {BF_cat=bf_cat(BF)}
          results_collect = rbind(results_collect, c(run_info, "LearninginLow", H1_simple_effect, BF, BF_cat, output2$coefficients["c_freqFclow:c_learn", "Pr(>|z|)"]))
          }
...
rbind(read.table(paste(localDir, "preReg/total/40.csv", sep = ""), header = T),
      ...
      read.table(paste(localDir, "preReg/total/120.csv", sep = ""), header = T),
      ...
      read.table(paste(localDir, "preReg/Half_beta2_half_beta3/120.csv", sep = ""), header = T))->results_collect
...
test_beta3 = subset(results_collect, round(alpha,3) == round(alphaX,3) & round(beta1,3)==round(beta1X,3)& round(beta2,3)==round(beta2X/2,3))
...
plottitle = "Interaction: Propotion BF>3 (100 runs) with half beta2 and half beta3 learning \n (original means is 1) "
...
pdf("interaction_halfbeta2_halfbeta3.pdf")
p
dev.off()
...
test_beta1 = subset(results_collect, round(results_collect$alpha,3) == round(alphaX,3) & round(results_collect$beta1,3)==round(beta1,3)& round(results_collect$beta3,3)==round(beta3/2,3)& round(results_collect$beta2,3)==round(beta2X/2,3))
test_beta1<-results_collect[round(results_collect$alpha,3) == round(alphaX,3) & round(results_collect$beta1,3)==round(beta1,3)& round(results_collect$beta3,3)==round(beta3/2,3)& round(results_collect$beta2,3)==round(beta2X/2,3),]
...
summary$H1sd = as.factor(round(summary$H1sd,1))
summary$H1sd = relevel(summary$H1sd, ref = "-0.9")
summary$TrueEffect = as.factor(round(summary$TrueEffect,1))
...
pdf("frequency_halfbeta2_halfbeta3.pdf")
p 
dev.off()
...
table(round(test_beta2$beta2,2))
mean(round(test_beta2$beta3,3))
...
# work with factor versions of the parameters rounded to 0.5/1 for printing purposes
summary$H1sd = as.factor(round(summary$H1sd,1))
summary$H1sd = relevel(summary$H1sd, ref = "0.3")
summary$TrueEffect = as.factor(round(summary$TrueEffect,1))
summary$TrueEffect = relevel(summary$TrueEffect, ref = "0.3")
...
pdf("mainLearning_halfbeta2_halfbeta3.pdf")
p
dev.off()
...
for (beta2_mean in beta2_means) {
  ...
  prop = 0
      ...
      prop = prop / nrow(data)
      summary <- rbind(summary, data.frame(TrueEffect=beta2_mean, H1sd=lfcond_means, N_subj=s, Prop_H1=prop))
    }
  }
}
...
# work with factor versions of the parameters rounded to 0.5/1 for printing purposes
summary$H1sd = as.factor(round(summary$H1sd,1))
summary$H1sd = relevel(summary$H1sd, ref = ".6")
summary$TrueEffect = as.factor(round(summary$TrueEffect,1))
summary$TrueEffect = relevel(summary$TrueEffect, ref = "0.3")
plottitle = "Simple Effect Learning in lowfreq: Propotion BF>3 (100 runs) with half beta2 and half beta3 means and H1s"
p <- ggplot(data=summary,aes(x=N_subj, y=Prop_H1, colour=TrueEffect, linetype=H1sd))
p
p <- p+ ggtitle(plottitle) +theme_bw() + geom_line(aes(color = TrueEffect))
p
...
pdf("LearningInLow_beta2Half_halfbeta3.pdf")
p
dev.off()
