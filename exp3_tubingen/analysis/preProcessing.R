...
#### load stimuli set ####
fribbleSet<-readxl::read_xlsx(paste(localGitDir, "/exp3_tubingen/stimuli/testing.xlsx", sep = ""),
                              sheet = 1)


fribbleSet_conJudg<-readxl::read_xlsx(paste(localGitDir, "/exp3_tubingen/stimuli/testing.xlsx", sep = ""),
                                      sheet = 3)

fribbleSet_learning<-read.csv(paste(localGitDir, "/exp3_tubingen/stimuli/learning.csv", sep = ""), stringsAsFactors = T)

#### load data ####
df <- list.files(paste(localGitDir, "/exp3_tubingen/data/", sep = "")); 

dataFLO <- data.frame()
for (i in 1:length(df)){
  read.csv(paste(localGitDir, "/exp3_tubingen/data/", df[i], sep = "", dec = "."),
           na.strings=c("","NA"),
           stringsAsFactors = T
           )-> temp
...
raw_data<- dataFLO[c('subject', 'language', 'trial_type' , 'codeword', 'rt',
...
write.csv(picLab, paste(localGitDir, "/exp3_tubingen/preProcessed_data/picLab.csv", sep = ""), row.names = F, quote = F)
...
write.csv(labPic, paste(localGitDir, "/exp3_tubingen/preProcessed_data/labPic.csv", sep = ""), row.names = F, quote = F)
...
#aggregate(response ~ trialType + correctFrequency, data = conJudg, mean)

write.csv(conJudg, paste(localGitDir, "/exp3_tubingen/preProcessed_data/contingency.csv", sep = ""), row.names = F, quote = F)
