...
group_by(labPic.clean, learning, correctFrequency) %>%
   summarise(
      count = n(),
      mean = mean(acc, na.rm = TRUE),
      sd = sd(acc, na.rm = TRUE)
...
group_by(picLab.clean, learning, correctFrequency) %>%
   summarise(
      count = n(),
      mean = mean(acc, na.rm = TRUE),
      sd = sd(acc, na.rm = TRUE)
...
plota<-ggviolin(df, x = "correctFrequency", y = "acc",
         add = "mean_sd", 
         error.plot = "crossbar",
          fill = "learning", 
...
ggplot(df, aes(x=correctFrequency, y=acc, fill = learning)) + 
...
plotb<-ggviolin(pf, x = "correctFrequency", y = "acc",
         add = "mean_sd", 
         error.plot = "crossbar",
         fill = "learning",
         palette = c("#00AFBB", "#E7B800"),
...
zf <- aggregate(acc ~ subjID+correctFrequency+learning, 
                data = FLO_tasks, mean)
plotc<-ggviolin(zf, x = "correctFrequency", y = "acc",
...
ggarrange(plota,plotb,plotc, ncol =3)->plot4
plot4


ggsave( "C:/Users/eva_v/Nexus365/Elizabeth Wonnacott - Eva_Liz_Leverhulme/leverhulmeNDL/fribbles/exp3_tubingen/figures/floReplication_violins.png", width =8, height = 5)
...
write.csv(labPic, "C:/Users/eva_v/Documents/University College London/Wonnacott, Elizabeth - Eva_Liz_Leverhulme/leverhulmeNDL/fribbles/exp3_tubingen/preProcessed_data/labPic_errorType.csv", row.names = F, quote = F)
write.csv(picLab, "C:/Users/eva_v/Documents/University College London/Wonnacott, Elizabeth - Eva_Liz_Leverhulme/leverhulmeNDL/fribbles/exp3_tubingen/preProcessed_data/picLab_errorType.csv", row.names = F, quote = F)
