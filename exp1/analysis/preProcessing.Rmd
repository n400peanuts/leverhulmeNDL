...
### Comparison by learning by tasks
...
timeslice_range <- pictureLabel[pictureLabel$rt > 200 ,] %>%
  filter(learning == "FL") %>%
  dplyr::mutate(RT_bin = cut(rt, breaks = break_seq)) %>%
  dplyr::group_by(RT_bin, category) %>%
  dplyr::mutate(RT_bin_avg = mean(rt, na.rm = T))
...
```{r}
aggregate(acc ~ subjID+learning, pictureLabel[pictureLabel$rt > 200 ,], mean)-> speedacc
aggregate(rt ~ subjID+learning, pictureLabel[pictureLabel$rt > 200,], mean)-> speedacc2
merge(speedacc, speedacc2, by =  c("subjID", "learning"))-> speedacc
