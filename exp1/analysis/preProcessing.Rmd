---
title: "FLO replication - Preprocessing and analysis"
author: "Eva"
date: "4/3/2020"
output:
  pdf_document:
...
source('C:/Users/eva_v/Documents/languagelearninglab/tools/loadFunctionsGithub.R')
...
#### People that scored less than 50% in >1 block:



```{r}
c("1422475", "1422689") -> dumbPeople
```
...
```{r}
round(nrow(generalizationPL[generalizationPL$rt<100,]) / nrow(generalizationPL)*100,2)
```
...
```{r}
round(nrow(generalizationLP[(is.na(generalizationLP$resp)),])/nrow(generalizationLP)*100,2)
```

How many trials were rt < 100?
```{r}
round(nrow(generalizationLP[generalizationLP$rt<100,])/ nrow(generalizationLP)*100,2)
```
...
timeslice_range <- labelPicture[labelPicture$rt > 100 &
                                         !(labelPicture$subjID %in% badsubjs) ,] %>%
  filter(learning == "FL") %>%
...
ggplot(aes(x=RT_bin_avg, y=mean, weight = n), 
  ...
  ggtitle('speed-accuracy tradeoff - FL - labelPicture')
...
ggplot(aes(x=RT_bin_avg, y=mean, weight = n), 
  ...
  ggthemes::theme_hc()+
  xlab("Average RT on trials") +
  ggtitle('speed-accuracy tradeoff LF - LabelPicture')
  ylab("Proportion Correct")
...
timeslice_range <- pictureLabel[pictureLabel$rt > 100 &
                                         !(pictureLabel$subjID %in% badsubjs),] %>%
  filter(learning == "FL") %>%
  dplyr::mutate(RT_bin = cut(rt, breaks = break_seq)) %>%
  dplyr::group_by(RT_bin, category) %>%
  dplyr::mutate(RT_bin_avg = mean(rt, na.rm = T))
...
ggplot(aes(x=RT_bin_avg, y=mean, weight = n), 
  ...
  ggthemes::theme_hc()+
  xlab("Average RT on trials") +
  ggtitle('speed-accuracy tradeoff - FL - pictureLabel')
  ylab("Proportion Correct")
...
ggplot(aes(x=RT_bin_avg, y=mean, weight = n), 
  ...
  ggthemes::theme_hc()+
  xlab("Average RT on trials") +
  ggtitle('speed-accuracy tradeoff - LF - pictureLabel')
  ylab("Proportion Correct")
...
### Comparisons by tasks + learning + frequency
...
```{r}
genTask_model <- glmer(acc ~  frequency.ct*learning.ct + task + (frequency.ct|subjID) , 
         data = genTask, 
         family="binomial",
...
genTask_bf = data.frame(
    ...
    meandiff = c(
      round(summary(genTask_model)$coefficients["frequency.ct:learning.ct", "Estimate"],3),
       ...
       round(summary(genTask_model)$coefficients["taskgeneralizationPL", "Estimate"],3)
       ),
    ...
    sd = c(
      round(summary(genTask_model)$coefficients["frequency.ct:learning.ct", "Std. Error"],3),
       ...
       round(summary(genTask_model)$coefficients["taskgeneralizationPL", "Std. Error"],3)
       )
)
...
## BF for Frequency:
```{r}
Bf(sd = genTask_bf[genTask_bf$condition=='frequency',]$sd, 
...
## BF for learning:
```{r}
Bf(sd = genTask_bf[genTask_bf$condition=='learning',]$sd, 
...
```{r}
Bf(sd = genTask_bf[genTask_bf$condition=='frequency by learning',]$sd, 
