...
public class XSTrainingComponent extends ExperimentComponent {
	...
	public XSTrainingComponent(ExperimentSequence es) {
		super(es);
	...
	public void startMeUp() {	
		new Thread() {
			public void run() {
				...
				try {
					sleep(5000);
					System.out.println(System.currentTimeMillis()+"\tBEGIN XS TRAIN");
				} catch (Exception exc) {
					exc.printStackTrace();
				}
				...
				while (imgBuffer.getCurrentImage() != null) {
					if (skipped) {
						XSTrainingComponent.this.complete();
						return;
					}
				...
				System.out.println(System.currentTimeMillis() + "\tEND XS TRAIN");
	...
	public void keyPressed(KeyEvent e) {
		switch (e.getKeyCode()) {
		case KeyEvent.VK_N:
			System.out.println(System.currentTimeMillis() + "\tXS TRAIN SKIPPED");
			skipped = true;
			break;
		case KeyEvent.VK_5:
			if (!running) {
	...
	class ImageBuffer {
		...
		public ImageBuffer() {
			MediaTracker mt = new MediaTracker(XSTrainingComponent.this);
			for (int i=0; i<img.length; i++) {
				imgIndex += 2;
				Image newImg = Toolkit.getDefaultToolkit().createImage(
					ClassLoader.getSystemResource("img/xs/tSlide"+imgIndex+".PNG")
				);
		...
		public void loadNextImage() {
			...
			java.net.URL imgURL = ClassLoader.getSystemResource("img/xs/tSlide"+imgIndex+".PNG");
			if (imgURL != null)
